...
for i in range(0,120):
    ...
    ax1.set_title('\n GFS_para Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/GFS_para_TyS/gfs_para_hrly_pytpe_tys_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()