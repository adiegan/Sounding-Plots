...
url = 'http://nomads.ncep.noaa.gov:80/dods/sref/sref'+mdate+'/sref_nmb_ctl_'+get_init_hr(hour)+'z'
...
mkdir_p(output_dir)
mkdir_p(output_dir+'/SREF_nmb_HoP') #create subdirectory to store GFS output like this
...
lats = np.arange(20,55, 0.369)
lons = np.arange(240,310, 0.378)

#ds = ds.sel(lat = lats, lon = lons)

#Now loop through the 84 forecast hours to make the plots
for i in range(0,84):
    ...
    #Plot 2m 32F Wetbulb Isotherm
    #twb_2m32 = ax1.contour(x,y,twb,colors='r', alpha = 0.5, levels = [32])
    ...
    ax1.set_title('\n SREF nmb Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    ax1.set_extent((270.25, 274.25, 35.2, 38.45))#, crs = zH5_crs)    # Set a title and show the plot
    ...
    for i in range(1,r):
        ...
        sound_temps = sound_temps.squeeze()
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/SREF_nmb_HoP/SREF_nmb_hrly_pytpe_hop_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()