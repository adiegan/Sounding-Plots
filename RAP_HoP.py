...
startTime=datetime.utcnow()-dt.timedelta(hours=2)
...
url = 'http://nomads.ncep.noaa.gov:80/dods/rap/rap'+mdate+'/rap_'+get_init_hr(hour)+'z'
...
mkdir_p(output_dir)
mkdir_p(output_dir+'/RAP') #create subdirectory to store RAP output like this
...
lats = np.arange(20,55, 0.185)
lons = np.arange(240,310, 0.192)
...
for i in range(0,36):
    ...
    smap.plot_soundings(fig,ax1,data['temperature'],data['rh'],36.9,87.5,'local',cape=True)
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/RAP/RAP_TEST_V6_hrly_pytpe_hop_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
