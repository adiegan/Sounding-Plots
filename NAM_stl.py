...
mkdir_p(output_dir)
mkdir_p(output_dir+'/NAM/STL') #create subdirectory to store GFS output like this
...
for i in range(0,84):
    ...
    ax1.set_extent((267.8, 271.8, 36.975, 40.225))#, crs = zH5_crs)    # Set a title and show the plot
    ...
    smap.plot_soundings(fig,ax1,data['temperature'],data['rh'],38.6,90.2,'local',cape=True)
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/NAM/STL/NAM_hrly_pytpe_stl_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
