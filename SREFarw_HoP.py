...
url = 'http://nomads.ncep.noaa.gov:80/dods/sref/sref'+mdate+'/sref_arw_ctl_'+get_init_hr(hour)+'z'
...
mkdir_p(output_dir)
mkdir_p(output_dir+'/SREF_arw_HoP') #create subdirectory to store GFS output like this
...
for i in range(0,84):
    ...
    ax1.set_title('\n SREF arw Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/SREF_arw_HoP/SREF_arw_hrly_pytpe_hop_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()